...
export class ChatListStateService {
    ...
    private findChatWindowStateIndexByContact(contact: Contact): number {
        return this.openChats$.getValue()
            .findIndex((chatWindowState) => contact);
            // return this.openChats$.getValue()
            // .findIndex((chatWindowState) => chatWindowState.contact.equalsBareId(contact));
    }

    private findChatWindowStateByContact(contact: Contact): ChatWindowState {
        return this.openChats$.getValue()
            .find((chatWindowState) => contact);
            // return this.openChats$.getValue()
            // .find((chatWindowState) => chatWindowState.contact.equalsBareId(contact));
    }
}
